<div class="w3-display-container" style="height: 500px">
	<div class="w3-row w3-padding-24">
		<div class="w3-center w3-margin">
			<h3><b>ĐỐI TÁC</b></h3>
			<p><b>Một số đối tác của chúng tôi</b></p>
		</div>
	</div>
	<div class="row w3-margin w3-padding-large">
		<div class="w3-quarter w3-padding ">
			<img src="../../img/partner/House3D.jpg" class="w3-image w3-border w3-hover-shadow" style="height: 150px;width: 100%;">
		</div>
		<div class="w3-quarter w3-padding">
			<img src="../../img/partner/sun.jpg" class="w3-image w3-border w3-hover-shadow" style="height: 150px;width: 100%;">
		</div>
		<div class="w3-quarter w3-padding">
			<img src="../../img/partner/fpt.jpg" class="w3-image w3-border w3-hover-shadow" style="height: 150px;width: 100%;">
		</div>
		<div class="w3-quarter w3-padding">
			<img src="../../img/partner/porn.png" class="w3-image w3-border w3-hover-shadow" style="height: 150px;width: 100%;">
		</div>
	</div>
</div>