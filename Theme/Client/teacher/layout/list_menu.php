<div class="w3-display-container">
    <div class="w3-col w3-padding-64 w3-green w3-dark-grey w3-menu" style="width:20%;height: 630px;position: fixed;">
        <div class="w3-padding w3-light-grey w3-margin w3-round" style="height: 100px;">
            <div class="w3-quarter">
                <img src="../../../img/slide/user.png" style="width:75%;height: 40px" class=" w3-left w3-round-xxlarge">
            </div>
            <div class="w3-threequarter w3-margin-top " style="font-size: 12px"><?php echo $_SESSION['full_name'] ?></div>
        </div>

        <div class="w3-grey w3-serif w3-large w3-light-grey w3-round w3-margin">
            <a href="teacher.php" class="w3-text-black" >
                <div class="w3-padding-large w3-hover-grey">
                    <span class="glyphicon glyphicon-dashboard w3-margin-right"></span> Dashboard
                </div>
            </a>
            <a href="teacher.php?detail=list_ability" class="w3-text-black" >
                <div class="w3-padding-large w3-hover-grey">
                    <span class="glyphicon glyphicon-dashboard w3-margin-right"></span>Từ điển năng lực
                </div>
            </a>
        </div>
    </div>
</div>